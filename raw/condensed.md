...
| **ZEST** | Willpower, endurance, spirit | Cannot gain/spend COURAGE |
| **WITS** | Cunning, perception, trickery | Feral, attack nearest creature each round until restrained |

Stat damage lowers your effective value (and save target). **Max stat value never changes.**

---

### TARGET NUMBERS & SAVES

Roll a save when success isn't guaranteed. Roll your dice pool, if the **Gob Die or any COURAGE die meets or beats the target number**, you succeed. **A 6 always succeeds** and cannot be discarded by STRESS.
...
**STRESS**, strain on your goblin. Soaks damage before stats take the hit.
- Mark 1 STRESS per 1 incoming damage (before it hits MEAT).
- If no STRESS is available, damage goes into MEAT (chronic damage).
- Max STRESS dice in pool = current level. Hindered attacks can exceed this cap.

**COURAGE**, boldness and goblin heart. Max = ZEST stat. Each point = one d6.

Gain COURAGE when:
- You succeed on the Gob Die
- Critical hit
- You take a risk the whole table agrees is stupid
- GM awards it for clever play, social victories, or genuine confidence

Spend COURAGE by adding d6s to your pool (max added = ZEST stat).
...
| Rest | Time | Effect |
|---|---|---|
| **Quick Rest** | 1 Turn (~10 min) | Clear d4+1 STRESS (d6+1 if eating). Increases Itch one step. |
| **Long Rest** | 1 Watch (~6 hrs) | Clear all STRESS. Restore d6+1 chronic damage from one stat. |
| **Full Rest** | 1 week in safety | Restore all STRESS and all stats. Removes most long-term effects. |

**Eating:** Must eat during at least 2 watches per day or gain **Hungry**. Rations have **(4\*)** uses.  
**Sleeping:** Must sleep at least 1 Watch per day or gain **Exhausted.**
...
Conditions are physical tokens. Every goblin has 2 **Guts slots** for conditions. If both full > go into equipment/pack slots. If no space available > drop an item or take **d12 chronic MEAT damage.** Conditions cannot be moved once placed.

Conditions can stack unless noted. A condition placed cannot be moved until removed.

| Condition | Effect | Clears |
|---|---|---|
| **Disoriented** | All rolls Hindered | Any rest |
| **Frightened** | Must make ZEST save to attack | Succeed on ZEST save or rest |
| **Chilled** | Take 1d4 ZEST damage (reduced by DEF) per turn | Warmed by fire during rest |
| **Slimed** | Covers a slot, item unusable | When cleaned |
| **Burning** | d6 MEAT damage at start of action | Doused or smothered |
| **Bleed** | 1 MEAT damage at start of action | Treated with bandages |
| **Blind** | WITS roll before any sight-dependent action | Washed out |
| **Immobilized** | Cannot move | ZEST save as action |
| **Charmed** | Cannot attack charmer | MEAT reduced or ZEST save as action |
| **Knocked** | Prone; attacks against are Bolstered | Standing up |
...
1. **Roll stats**, 3d6, discard lowest, down the line: MEAT > ZEST > WITS. Outline max circles in pen.
2. **Roll STRESS Threshold**, 1d6. If ZEST 9+, roll 2d6 discard lowest. Outline max stress in pen.
3. **Roll starting COURAGE**, 1d4. Place that many d6 on COURAGE slot.
4. **Determine Background**, cross-reference highest stat with starting COURAGE. Ties = player chooses.
5. **Take Background items** (two class-based items).
6. **Take starting gear**, rations, torch, 15ft rope, 1 weapon + 1 Hoard item. Pack size by background.
7. **Roll Name**, d66 twice, combine.
8. **Roll Physical Trait**, d66 once.
9. **Roll Quirk**, d66 once.
...
3. Roll attack pool: weapon dice + COURAGE dice (no Gob Die; no STRESS unless Hindered).
4. Assign one die as **Break die** and one as **Damage die.** COURAGE dice can go to either.
5. If Hindered: discard dice matching STRESS die (unless critical hit).
6. Break die must beat target DEF to land.
7. Damage > Shield absorbs first (1 use per 1 damage); excess or unblocked > target STRESS.
8. STRESS full > MEAT takes damage. **3+ MEAT from one hit** > roll Vital Blow.
9. MEAT hits 0 > death.

**Hindered in combat:** Add 1 STRESS die to attack pool. Multiple hinders still only add 1. Affects entire pool.  
**Bolstered in combat:** Add 1 free COURAGE die to attack pool.
...
When a player takes **3+ MEAT damage from one attack**, make a MEAT save.

| Result | Effect |
|---|---|
| Success | Take the hit clean. |
| Failure | Roll d6 on Injury Table (unless source has its own Vital Blow effect). |

**Injury Table (d6):**

| Roll | Result |
|---|---|
| 1 | Permanent Injury, see below |
| 2 | Unconscious |
| 3–6 | Flesh Wound (1 chronic MEAT damage) |

**Permanent Injuries (d4):**

| Roll | Injury | Slot Disabled |
|---|---|---|
...
| Ranged Heavy | rd6+rd8 | [11] | Requires equipped ammo (e.g. quiver) |
...
Scrolls are [1] with 3 uses. Committing dice marks uses. All uses = scroll turns to ash.
...
6. Spell casts at declared tier.
...
Starts at **20.** Represents the dungeon waking up.
...
**Noise:** Significant noise (fight, dropped torch) = roll Itch die immediately, in addition to any Turn.

**Light:** Torches **(4\*)**, mark use every 3 Turns. In darkness: all saves Hindered + gain Rattled on failed save. Lanterns provide steady light; need oil to restock.

**Locks:** MEAT save. Without appropriate tools: TN +2.

**Traps:** Relevant stat save based on approach. Noticing = WITS. Disarming = MEAT. Smashing = ZEST. Always foreshadow traps before they trigger.
...
Results shift based on party behavior.
...
5×5 hex flower. City at centre. Inner ring = 6 hexes (easy reach). Outer ring = 12 hexes (expedition territory).

**Travel Sequence (each Watch):**
1. Pick direction.
2. Roll weather.
3. WITS save to navigate. Fail = drift to adjacent hex.
4. Roll d6 encounter die. 1 = encounter, 2 = foreshadow.
...
**Weather (roll d6 per Watch by season):**
...
| 4 | Grey and damp | No effect. Miserable. |
...
| 2 | Thunderstorm | Nav Hindered; No shelter means no Rest. |
...
| d6 | Weather | Effect |
|---|---|---|
| 1 | Early frost | Arrive Chilled; fire req. for Long Rest. |
...
| Roll | Weather | Effect |
|---|---|---|
| 1 | Blizzard | can't move; caught outside: use ration, mark 1 STRESS, gain Chilled. |
| 2 | Heavy snow | Nav Hindered; if off-road +1 Watch. |
...
**Camping:** 1 Watch, counts as Long Rest. Roll encounter die at end. Fire is warm but visible.
...
| 17 | Deliver a message | Message will give them more reasons to distrust goblins |
...
*(Regional modifiers in full tables section.)*
...
Loyalty +1: pay more than asked, send business, do genuine favour.  
Loyalty −1: stiff them, bring heat, ask for something costly.

**Contacts and Standing:** Contact hit 0 = Gobfather hears about it, drop one Standing step. Bad Standing = all contact costs up, Loyalty can't improve.
...
Then make a ZEST save:
- **Success:** Gain 1 COURAGE per d10 rolled. Gain a contact.
- **Failure:** Gain 1 COURAGE. Roll Trouble Table (d12).
...
Odd jobs in the City. Describe it. Make a WITS save.
...
## SECTION 7: RUNNING THE GAME FOR GM'S

### The Itch in Context

- **Foreshadow before punishing.** Itch hits 3 = something changes (sounds, a patrol turns, a torch dies).
- **Encounters aren't always combat.** A patrol that walks past hiding goblins is more tense than one that charges.
- **Noise has consequences.** A bomb should wake the floor.

### Running Social Encounters

- **Reaction roll = opening position, not outcome.** Player behaviour shifts results.
- **Saves are for pressure.** Good argument + plausible offer = no roll needed.
- **Failed rolls = complications, not dead ends.** Something still happens.
- **COURAGE rewards cleverness.** Award it for social victories that feel earned.
...
| d6 | Contents |
|---|---|
| 1 | Empty. Evidence of what was here. |
| 2 | Hazard (trap, bad air, rising water). |
| 3 | Hostile creature. Roll encounter table. |
| 4 | Neutral/unaware creature. Roll Reaction. |
| 5 | Treasure. Roll loot table. |
| 6 | Creature + treasure. Guarding or oblivious. |
...
| 5 | Barred from other side. Must force or circumvent. |
...
**Placing Treasure:**
- One significant piece per dungeon.
- Scatter dungeon valuables throughout.
- Best loot in most dangerous locations.
- Magic items rare.

**Factions in a Dungeon (each needs):**
- Who they are and roughly how many.
- What area they're in.
- Reaction to intruders.
- Goals, resources, fears.

Clever goblins should be able to play factions against each other.

### Session Zero Checklist

- How lethal do you want it?
- How much of the City vs dungeon?
- What is the Gobfather like at your table?
- Do they start with a safe house or earn one?
- Don't introduce the Gobfather in person, let him be a presence first.
- Start in motion. Give them something to react to immediately.

---

# TOME 2: THE WORLD AT LARGE
